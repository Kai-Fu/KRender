...
bool PointLightBase::Save(FILE* pFile)
{
	std::string typeName = "PointLightBase";
	if (!SaveArrayToFile(typeName, pFile))
		return false;

	if (!SaveTypeToFile(mPos, pFile))
		return false;

	if (!SaveTypeToFile(mLightMat, pFile))
		return false;

	if (!SaveTypeToFile(mIntensity, pFile))
		return false;

	return true;
}

bool PointLightBase::Load(FILE* pFile)
{
	std::string srcTypeName;
	std::string dstTypeName = "PointLightBase"; 
	if (!LoadArrayFromFile(srcTypeName, pFile))
		return false;
	if (srcTypeName.compare(dstTypeName) != 0)
		return false;

	if (!LoadTypeFromFile(mPos, pFile))
		return false;

	if (!LoadTypeFromFile(mLightMat, pFile))
		return false;

	if (!LoadTypeFromFile(mIntensity, pFile))
		return false;

	return true;
}

bool RectLightBase::Save(FILE* pFile)
{
	std::string typeName = "RectLightBase";
	if (!SaveArrayToFile(typeName, pFile))
		return false;

	if (!SaveTypeToFile(mParams, pFile)) 
		return false;

	return true;
}

bool RectLightBase::Load(FILE* pFile)
{
	std::string srcTypeName;
	std::string dstTypeName = "RectLightBase"; 
	if (!LoadArrayFromFile(srcTypeName, pFile))
		return false;
	if (srcTypeName.compare(dstTypeName) != 0)
		return false;

	if (!LoadTypeFromFile(mParams, pFile))
		return false;

	return true;
}

bool LightScheme::Save(FILE* pFile)
{
	std::string typeName = "LightScheme";
	if (!SaveArrayToFile(typeName, pFile))
		return false;

	UINT64 lightCnt = mpLights.size();
	if (!SaveTypeToFile(lightCnt, pFile))
		return false;
	for (std::vector<ILightObject*>::iterator it = mpLights.begin(); 
		 it != mpLights.end(); ++it) {
		std::string lightType = (*it)->GetType();
		if (!SaveArrayToFile(lightType, pFile))
			return false;
...
bool LightScheme::Load(FILE* pFile)
{
	ClearLightSource();
	std::string srcTypeName;
	std::string dstTypeName = "LightScheme"; 
	if (!LoadArrayFromFile(srcTypeName, pFile))
		return false;
	if (srcTypeName.compare(dstTypeName) != 0)
		return false;
	...
	UINT64 lightCnt = 0;
	if (!LoadTypeFromFile(lightCnt, pFile))
		return false;
	for (UINT32 i = 0; i < (UINT32)lightCnt; ++i) {
		std::string lightType;
		if (!LoadArrayFromFile(lightType, pFile))
			return false;

		ILightObject* pLight = CreateLightSource(lightType.c_str());
		if (pLight) {
			if (!pLight->Load(pFile))
				RemoveLightSource(pLight);
