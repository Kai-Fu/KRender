...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        switch(buf[0]) {
        case '#':               /* comment */
            /* eat up rest of line fgets(buf, sizeof(buf), file);*/
            /* the commented line may contain some extension of material definition */
            break;
        case 'n':               /* newmtl */
    ...
    for (i = 0; i < nummaterials; i++) {
		...
		model->materials[i].transparency[0] = 1.0;
		model->materials[i].transparency[1] = 1.0;
		model->materials[i].transparency[2] = 1.0;
    }
    ...
    while(fscanf(file, "%s", buf) != EOF) {
		...
		case 'm':	/* map_xxx */
			if (0 == strcmp(buf, "map_Kd")) {
				fgets(buf, sizeof(buf), file);
				sscanf(buf, "%s", buf);
				model->materials[nummaterials].diffuse_map = _strdup(buf);
			}
			break;
        case 'K':
            switch(buf[1]) {
		...
		case 'T':
			...
			case 'f':
				fscanf(file, "%f %f %f",
					&model->materials[nummaterials].transparency[0],
					&model->materials[nummaterials].transparency[1],
					&model->materials[nummaterials].transparency[2]);
				break;
			}
			break;
		default:
...
glmSecondPass(GLMmodel* model, FILE* file) 
{
    ...
    int v, n, t;
    char buf[128];
	GLuint sm_group;			/* current smooth group */
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'v':               /* v, vn, vt */
			...
			case 's':				/* smooth group */
				/* eat up rest of line */
				fgets(buf, sizeof(buf), file);
				sscanf(buf, "%d", &sm_group);
				break;
            case 'f':               /* face */
                v = n = t = 0;
                fscanf(file, "%s", buf);
				T(numtriangles).mindex = material;
				T(numtriangles).sm_group = sm_group;
                /* can be one of %d, %d//%d, %d/%d, %d/%d/%d %d//%d */
                if (strstr(buf, "//")) {
                    /* v//n */
                    sscanf(buf, "%d//%d", &v, &n);
  ...
  model->has_smooth_group = (sm_group != 0xffffffff);
