...
void GetConvertedMaterials(FbxNode* pFbxNode, std::vector<ShaderHandle>& convertedMtls)
{
	...
	for (int m_i = 0; m_i < matCnt; m_i++){
		...
		if (pMtl) {
			pPhongSurf = KRT_CreateSurfaceMaterial("simple_phong_default.template", pMtl->GetName());
			float tmpClr[3];
			float tmpScale;
			...
			pPhongSurf = KRT_CreateSurfaceMaterial("simple_phong_default.template", wire_color_mtl_name.c_str());
