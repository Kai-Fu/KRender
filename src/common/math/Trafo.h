#ifndef _nvsg_nvmath_Trafo_
#define _nvsg_nvmath_Trafo_
...
namespace nvmath
{
  ...
  class Trafo
  {
    public:
      ...
      //! Copy Constructor
      Trafo( const Trafo &rhs  //!< Trafo to copy
                    );
       ...
       *  \endcode
       *  \sa getMatrix */
      Mat44f getInverse( void ) const;

      //! Set the center of ration of the transformation.
      void setCenter( const Vec3f &center              //!<  center of rotation
                             );
      ...
      void setOrientation( const Quatf &orientation    //!<  rotational part of transformation
                                  );

      //! Set the scale orientational part of the transformation.
      void setScaleOrientation( const Quatf &scaleOrientation  //!< scale orientational part of transform
                                       );

      //! Set the scaling part of the transformation.
      void setScaling( const Vec3f &scaling            //!<  scaling part of transformation
                              );

      //! Set the translational part of the transformation.
      void setTranslation( const Vec3f &translation    //!<  translational part of transformation
                                  );
      ...
      void setMatrix( const Mat44f &matrix   //!< complete transformation
                             );

      //! Copy operator.
      Trafo & operator=( const Trafo &rhs //!< Trafo to copy
                       );

      //! Equality operator.
      /** \returns \c true if \c this is equal to \a t, otherwise \c false  */
      bool operator==( const Trafo &t    //!<  \c Trafo to compare with
                              ) const;

      bool operator!=( const Trafo &t ) const;
  ...
  Trafo lerp( float alpha      //!<  interpolation parameter
                     , const Trafo &t0  //!<  starting value
                     , const Trafo &t1  //!<  ending value
                     );

  void lerp( float alpha, const Trafo & t0, const Trafo & t1, Trafo & tr );
