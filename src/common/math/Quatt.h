#ifndef _nvsg_nvmath_Quatt_
#define _nvsg_nvmath_Quatt_
...
namespace nvmath
{
  ...
  template<typename T>
    bool isNormalized( const Quatt<T> & q, T eps = std::numeric_limits<T>::epsilon() )
  {
  ...
  inline void decompose( const Quatt<T> & q, Vecnt<3,T> & axis, T & angle )
  {
	...
	||	 abs((angle - PI)) < std::numeric_limits<T>::epsilon()
	||	 abs((angle + PI)) < std::numeric_limits<T>::epsilon() )
    {
