#ifndef _nvsg_nvmath_Vecnt_
#define _nvsg_nvmath_Vecnt_
...
namespace nvmath
{
   ...
   *  \return \c true, if \ v0 and \a v1 differ less than \a eps in each component, otherwise \c
   *  false.
  ...
  template<unsigned int n, typename T>
    bool isNormalized( const Vecnt<n,T> & v, T eps = std::numeric_limits<T>::epsilon() )
  {
   ...
   *  \sa operator/() */
  template<unsigned int n, typename T>
    Vecnt<n,T> operator*( const Vecnt<n,T> & v, T s );
   ...
   *  \sa operator/() */
  template<unsigned int n, typename T>
    Vecnt<n,T> operator*( T s, const Vecnt<n,T> & v );
  ...
  *  \a creaseAngle (in radians), their normals are set to the average of their normals. */
  ...
  {
    bool eq = true;
    for ( unsigned int i=0 ; eq && i<n ; ++i )
    {
      eq = ( abs( v0[i] - v1[i] ) < eps );
    }
    return( eq );
  }
  ...
  inline bool  operator==( const Vecnt<n,T> & v0, const Vecnt<n,T> & v1 )
  {
    ...
    for ( unsigned int i=0 ; eq && i<n ; ++i )
    {
      eq = ( abs( v0[i] - v1[i] ) < std::numeric_limits<T>::epsilon() );
    }
    return( eq );
  }
  ...
  template<unsigned int n, typename T>
  inline Vecnt<n,T>  operator*( const Vecnt<n,T> & v, T s )
  {
    Vecnt<n,T> ret(v);
    ret *= s;
    return( ret );
  ...
  template<unsigned int n, typename T>
  inline Vecnt<n,T>  operator*( T s, const Vecnt<n,T> & v )
  {
    return( v * s );
  }
